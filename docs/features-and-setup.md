...
| **Comms alias → name** | The local node maps a running Claude/Codex harness session to its agent identity and alias. The viewer uses that immutable session binding for names and receiver status; reused aliases cannot lend their status to an older session. Legacy alias events remain readable during migration. |
